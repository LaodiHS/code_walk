// `<link rel="stylesheet" href="static/${copy}/${file}"></link>`
const min = {}
class GenerateDomResources {
  constructor() {
    ...
    this.dependency = {
      ...
      styles: [],
      css: (copy, file) =>
        (this.dependency.styles.push(`<meta theme="${file.split('.').shift()}">`)),
    };
  }
  ...
  async generateScripts(srcObjectPathStr) {
    try {
      ...
      const filter = [".map", "types.js", "utils.js", "gridstack.js", "utils_1.js", "gridstack-engine.js", "gridstack-ddi.js", "gridstack-dd.js", "gridstack.css", "gridstack-extra.css", "LICENSE.txt"];
      ...
      while (files.length) {
        ...
        if (filter.some(script => scriptsNode.includes(script))) continue;

        if (srcPathStr.includes('theme')) {
